...
public abstract class HttpMethod implements HttpConnection {
    ...
    public void buildCookies( String cookiesData ) {
    ...
    public void buildArguments( String argumentsData ) {

        this.argumentsData = argumentsData;

    }

    public void buildHeaders( Map<String, String> headerMap ) {
    ...
    public void connect( String remote, int port ) throws IOException {
        ...
        for ( System.out.println( Conf.getProperty( PRINT.INFO_PREFIX ) + "开始连接 " + remote + ":" + port ); true; ) {
    ...
    public String receive() throws IOException {
        ...
        for ( boolean startFlag = false; true; ) {
            ...
            if ( byteNum != 0 ) {
                ...
                timeoutStart = ( timeoutStart == 0 ) ? System.currentTimeMillis() : timeoutStart;
    ...
    private String buildHeaderString() {
        ...
        Iterator<String> iterator = this.headerMap.keySet().iterator();

        for ( String key = null; iterator.hasNext(); ) {
            key = iterator.next();
            temp.append( key + ": " + this.headerMap.get( key ) + this.linebreak );
        }
    ...
    protected String buildArgumentString() {
        ...
        if ( ( null != this.argumentsMap ) && ( null != this.argumentsData ) ) {
            throw new BuildArgumentsException( "HttpMethod.buildArguments(),HttpMethod.buildArgumentString() 只能使用其中一个" );
        }

        String temp = null;
        if ( null != this.argumentsMap ) {
            temp = this.argumentsMap.toString();
        ...
        else if ( null != this.argumentsData ) {
            temp = this.argumentsData;
            this.argumentsData = null;
        }
        return ( null == temp ) ? "" : temp;
    }

    /**
     * Http请求原型： < request line > < http header > < blank line > < request body
     * >
     */
